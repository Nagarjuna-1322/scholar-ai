...
import { sampleScholarships } from "@/lib/data";
...
export function ChatAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { sender: 'bot', text: "Hi! How can I help? Try asking 'which scholarships am I eligible for?'" }
  ]);
  const scrollAreaRef = useRef<HTMLDivElement>(null);

  const recommendations = useMemo(() => {
    return sampleScholarships.filter(s => s.eligible_courses.includes(profile.course) && profile.income <= s.income_limit);
  }, [profile]);
  ...
  function chatbotReply(input: string): string {
    ...
    return "Sorry, I can only answer questions about eligibility and deadlines. Try asking: 'Any upcoming deadlines?'";
  }
